    ...
    return (
      <div className={classes.root}>
        ...
        >
          ...
          <List style={{backgroundColor:'#2b60ad'}}>
            {
              menuItems.map((text, index) => {
                return (
                  <Link to={text.link} style={{ textDecoration: "none", color: "#fff" }}>
                    <ListItem button divider key={text.title} >
                      <ListItemIcon style={{ textDecoration: "none", color: "#fff" }}>
                        <Tooltip title={text.title} placement="right">
                        <i className={`bi ${text.icon}`}></i>
                        </Tooltip>
                      </ListItemIcon>
